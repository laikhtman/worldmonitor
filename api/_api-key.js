...
const BROWSER_ORIGIN_PATTERNS = [
  /^https:\/\/(.*\.)?intelhq\.io$/,
  /^https:\/\/intelhq-[a-z0-9-]+-elie-[a-z0-9]+\.vercel\.app$/,
...
export function validateApiKey(req) {
  const key = req.headers.get('X-IntelHQ-Key');
  const origin = req.headers.get('Origin') || '';

  // Desktop app — always require API key
  if (isDesktopOrigin(origin)) {
    if (!key) return { valid: false, required: true, error: 'API key required for desktop access' };
    const validKeys = (process.env.INTELHQ_VALID_KEYS || '').split(',').filter(Boolean);
    if (!validKeys.includes(key)) return { valid: false, required: true, error: 'Invalid API key' };
    return { valid: true, required: true };
  }

  // Trusted browser origin (intelhq.io, Vercel previews, localhost dev) — no key needed
