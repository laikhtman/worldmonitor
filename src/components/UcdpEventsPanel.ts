...
export class UcdpEventsPanel extends Panel {
  ...
  private windowedList: WindowedList<UcdpGeoEvent> | null = null;
  ...
  /** PERF-008: Initialize windowed list for virtual scrolling of event rows */
  private initWindowedList(): void {
    this.windowedList = new WindowedList<UcdpGeoEvent>(
      {
  ...
  /** Render a single event row as HTML */
  private renderEventRow(e: UcdpGeoEvent): string {
  ...
  private renderContent(): void {
    ...
    if (filtered.length > VIRTUAL_SCROLL_THRESHOLD && this.windowedList) {
      this.setContent(`
      ...
      this.windowedList.setItems(filtered);
    } else {
      ...
      if (displayed.length === 0) {
        ...
        const rows = displayed.map(e => this.renderEventRow(e)).join('');
