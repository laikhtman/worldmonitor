...
import { APP_ORIGIN, APP_TITLE } from '@/config/branding';
...
export function generateStoryDeepLink(
  ...
  return `${APP_ORIGIN}/api/story?${params.toString()}`;
}
...
export const shareTexts = {
  ...
  linkedin: (data: StoryData) =>
    ...
    `\nData via ${APP_TITLE} - Open source geopolitical intelligence`,
...
export function getShareUrls(data: StoryData): Record<string, string> {
  ...
  return {
    ...
    reddit: `https://reddit.com/submit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(`${data.countryName} Intelligence Brief - ${APP_TITLE}`)}`,
